...
RESULT_ROOT = r"/workspace/examples/PythonProject1/pest_experiment_results_yolov8m_cbam"
SEED = 42
DEVICE = torch.device("cuda" if torch.cuda.is_available() else "cpu")
print(f"使用设备: {DEVICE}")

# ====================== 续训配置（续训新增） ======================
RESUME_TRAINING = True  # 是否启用续训
LAST_WEIGHT_PATH = os.path.join(RESULT_ROOT, "yolov8m_cbam_large_obj", "weights", "last.pt")  # last.pt路径
...
LARGE_OBJECT_CFG = {
    "enabled": True,
    "large_obj_threshold": 0.3,  # 面积占比>30%视为大目标
    "box_loss_weight": 5.0,      # 增大框回归损失权重
    "cls_loss_weight": 0.7,      # 优化分类损失
    "iou_loss_type": "CIoU",     # 更适合大目标的CIoU损失
    "conf_penalty": 0.1          # 大目标置信度惩罚
}

# 自适应学习率调度（创新点）
ADAPTIVE_LR_CFG = {
    "enabled": True,
    "lr_patience": 6,            # 性能停滞6轮则衰减
    "lr_decay_rate": 0.75,       # 衰减系数
    "min_lr": 1e-6,              # 最小学习率
    "monitor_metric": "mAP50"    # 基于mAP50自适应
}
...
CLASS_BALANCE_CFG = {
    "enabled": True,
    "alpha": 0.2,                # 采样概率调整系数
    "min_samples": 0             # 最小样本数阈值
}

# 知识蒸馏配置（创新点）
DISTILLATION_CFG = {
    "enabled": True,
    "teacher_model": "yolov8x.pt",  # 大模型作为教师
    "distill_weight": 0.25,         # 蒸馏损失权重
    "temperature": 2.5              # 软化温度
}
...
class AdaptiveLRScheduler:
    ...
    def step(self, current_metric):
        ...
        if self.counter >= self.cfg["lr_patience"]:
            ...
            self.counter = 0
            print(f" 学习率自适应调整：{[round(lr, 7) for lr in self.current_lr]}")
...
# ====================== 核心训练函数 ======================
def train_yolov8m_cbam(config_path):
    print("\n ====================== YOLOv8m + CBAM 大目标害虫检测训练 ======================")
    
    # 续训逻辑：检查last.pt是否存在（续训新增）
    resume_path = None
    if RESUME_TRAINING and os.path.exists(LAST_WEIGHT_PATH):
        resume_path = LAST_WEIGHT_PATH
        print(f"\n ✅ 检测到续训权重文件：{resume_path}")
        print(f" 🚀 将从断点继续训练（保留所有创新模块）")
    else:
        if RESUME_TRAINING:
            print(f"\n ⚠️ 未找到续训权重文件 {LAST_WEIGHT_PATH}")
            print(f" 🚀 将从头开始训练")
        else:
            print(f"\n 🚀 将从头开始训练（未启用续训）")
    
    # 加载模型：续训则加载last.pt，否则加载基础模型（续训新增）
    if resume_path:
        print("🔧 正在加载续训模型（包含CBAM权重）...")
        base_model = YOLO(resume_path)  # 加载续训模型
    else:
        print("🔧 正在加载YOLOv8m模型...")
        base_model = YOLO("yolov8m.pt")  # 加载基础模型
    
    # ========== 核心创新：Forward Hook集成CBAM（非侵入式） ==========
    # 初始化CBAM模块
    cbam_module = CBAM(1024).to(DEVICE)
    
    # 保存原始forward方法
    original_forward = base_model.model.forward
    
    # 定义兼容所有参数的forward方法（修复参数错误）
    def forward_with_cbam(x, *args, **kwargs):
        """兼容Ultralytics所有参数的CBAM集成方法"""
        # 注册Forward Hook自动应用CBAM
        hook_handles = []
        
        def cbam_hook(module, input, output):
            """在1024通道特征层应用CBAM注意力（核心创新）"""
        ...
        for name, module in base_model.model.named_modules():
            if hasattr(module, 'out_channels') and module.out_channels == 1024:
                handle = module.register_forward_hook(cbam_hook)
    ...
    # 替换模型forward方法（续训后仍需挂载CBAM）
    base_model.model.forward = forward_with_cbam
    
    print(f"\n ✅ 已成功为YOLOv8m添加CBAM注意力模块（非侵入式集成）")
    
    param_info = get_model_parameters(base_model.model)
    print(f"\n 模型参数信息：")
    print(f"   - 总参数：{param_info['总参数(M)']}M")
    print(f"   - 可训练参数：{param_info['可训练参数(M)']}M")
    print(f"   - ✅ CBAM注意力模块已成功加载")
    ...
    train_config = {
        ...
        "project": RESULT_ROOT,
        "name": "yolov8m_cbam_large_obj",
        "exist_ok": True,  # 续训时允许覆盖原有实验目录
        "seed": SEED,
        "conf": CONF_THRESHOLD,
        "iou": IOU_THRESHOLD,
        "verbose": True,
        ...
        # 续训核心参数（续训新增）
        "resume": resume_path if resume_path else False,  # 指定续训权重路径
    }
    ...
    if resume_path:
        print("\n 🚀 开始YOLOv8m+CBAM大目标害虫检测续训（集成多项创新策略）...")
    else:
        print("\n 🚀 开始YOLOv8m+CBAM大目标害虫检测训练（集成多项创新策略）...")
    train_results = base_model.train(**train_config)
    
    # 初始化自适应LR调度器
    lr_scheduler = AdaptiveLRScheduler(ADAPTIVE_LR_CFG)
    if ADAPTIVE_LR_CFG["enabled"]:
        lr_scheduler.init_optimizer(base_model)
    
    # 保存最佳模型
    best_model_src = os.path.join(RESULT_ROOT, "yolov8m_cbam_large_obj", "weights", "best.pt")
    best_model_dst = os.path.join(RESULT_ROOT, "model", "yolov8m_cbam_large_obj_best.pt")
    ...
    if os.path.exists(best_model_src):
        ...
        print(f" ⚠️ 最佳模型文件不存在：{best_model_src}")
        model_size_mb = 0.0
    
    # 模型验证
    best_model = YOLO(best_model_dst) if model_size_mb > 0 else base_model
    
    # 为验证模型也添加兼容的CBAM forward
    cbam_module_val = CBAM(1024).to(DEVICE)
    original_forward_val = best_model.model.forward
    
    def forward_with_cbam_val(x, *args, **kwargs):
        """验证阶段兼容所有参数的CBAM forward"""
        hook_handles = []
        
        def cbam_hook_val(module, input, output):
            if output.shape[1] == 1024:
                return cbam_module_val(output)
            return output
        
        for name, module in best_model.model.named_modules():
            if hasattr(module, 'out_channels') and module.out_channels == 1024:
                handle = module.register_forward_hook(cbam_hook_val)
                hook_handles.append(handle)
        
        try:
            result = original_forward_val(x, *args, **kwargs)
        finally:
            for handle in hook_handles:
                handle.remove()
    ...
    best_model.model.forward = forward_with_cbam_val
    ...
    best_model.eval()
    with torch.no_grad():
        for img_path in tqdm(val_img_paths, desc=" ⏱️ 推理速度测试"):
            if not os.path.exists(img_path):
                continue
    ...
    metrics = {
        ...
        "续训信息": {  # 续训新增
            "是否续训": bool(resume_path),
            "续训权重路径": resume_path if resume_path else "无",
            "续训起始Epoch": "自动识别" if resume_path else "0"
        }
    }
    
    # 保存指标
    metrics_path = os.path.join(RESULT_ROOT, "metrics", "yolov8m_cbam_large_obj_metrics.json")
    ...
    metrics_flat = {
        ...
        "是否续训": "是" if resume_path else "否",  # 续训新增
        "续训权重路径": resume_path if resume_path else "无"  # 续训新增
    }
    pd.DataFrame([metrics_flat]).to_csv(
        os.path.join(RESULT_ROOT, "metrics", "yolov8m_cbam_large_obj_metrics.csv"),
    ...
    print("\n" + "="*80)
    if resume_path:
        print(" 🎉 YOLOv8m+CBAM 大目标害虫检测续训完成！（集成多项创新策略）")
    else:
        print(" 🎉 YOLOv8m+CBAM 大目标害虫检测训练完成！（集成多项创新策略）")
    print("="*80)
    print(f"\n 📈 最终评估结果：")
    ...
    if resume_path:  # 续训新增
        print(f"    续训信息: 从 {resume_path} 恢复训练")
    print(f"\n 🔧 创新模块启用状态：")
    print(f"    - CBAM注意力机制: ✅ 开启（非侵入式集成）")
    print(f"    - 大目标害虫优化: {'✅ 开启' if LARGE_OBJECT_CFG['enabled'] else '❌ 关闭'}")
    print(f"    - 自适应学习率: {'✅ 开启' if ADAPTIVE_LR_CFG['enabled'] else '❌ 关闭'}")
    print(f"    - 类别平衡采样: {'✅ 开启' if CLASS_BALANCE_CFG['enabled'] else '❌ 关闭'}")
    print(f"    - 知识蒸馏增强: {'✅ 开启' if DISTILLATION_CFG['enabled'] else '❌ 关闭'}")
    print(f"\n 📁 结果文件保存至：{RESULT_ROOT}")
...
if __name__ == "__main__":
    try:
        ...
        # 开始训练/续训
        final_metrics = train_yolov8m_cbam(config_path)
        print("\n 🎯 所有任务完成！")
    except Exception as e:
        print(f"\n ❌ 训练过程出错：{str(e)}")
        import traceback
